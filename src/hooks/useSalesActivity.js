...
export function useSalesActivity(initialTimeframe = 'today') {
  ...
  const parseRecordDate = (dateStr) => {
    ...
    // Assume YYYY-MM-DD format (Supabase/ISO)
   ...
   * Handles both MM/DD/YYYY (FileMaker) and YYYY-MM-DD (Supabase) formats
  ...
  const filterRecordsByTimeframe = (records, timeframe) => {
    ...
    const filteredRecords = records.filter(record => {
      const recordDateStr = normalizeDateToISO(record.date);
      const recordDate = parseRecordDate(record.date);
    ...
    switch (timeframe) {
      case 'today': {
        ...
        records.forEach(record => {
          ...
          const recordDateISO = normalizeDateToISO(record.date);
          // Only process records from today
          if (recordDateISO === todayStr) {
      ...
      case 'thisWeek': {
        ...
        records.forEach(record => {
          // Normalize and parse the record date
          const recordDateISO = normalizeDateToISO(record.date);
          const [year, month, day] = recordDateISO.split('-').map(num => parseInt(num, 10));
      ...
      case 'thisMonth': {
        ...
        records.forEach(record => {
          ...
          if (recordDateISO.startsWith(thisMonthStr)) {
            // Extract day from date (format: YYYY-MM-DD)
            const day = parseInt(recordDateISO.split('-')[2], 10);
      ...
      case 'thisYear': {
        ...
        records.forEach(record => {
          ...
          if (recordDateISO.startsWith(`${currentYear}`)) {
            // Extract month from date (format: YYYY-MM-DD)
            const month = parseInt(recordDateISO.split('-')[1], 10) - 1; // Convert to 0-indexed
  ...
  const calculateMonthlyTotals = (records, timeframe) => {
    ...
    records.forEach(record => {
      ...
      if (recordDateISO.startsWith(`${currentYear}`)) {
        // Extract month from date (format: YYYY-MM-DD)
        const month = parseInt(recordDateISO.split('-')[1], 10); // Already 1-based
